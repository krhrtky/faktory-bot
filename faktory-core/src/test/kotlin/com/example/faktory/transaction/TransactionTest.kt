...
class TransactionTest : JooqTestBase() {
    ...
    fun `rollbackToSavepoint rolls back to savepoint`() {
        dsl.transaction { config ->
            ...
            val countBefore = txDsl.fetchCount(com.example.faktory.test.jooq.tables.Users.USERS)

            tx.savepoint("sp1")

            txDsl.execute("INSERT INTO users (name, email, age) VALUES ('User2', 'user2@example.com', 30)")
            val countAfter = txDsl.fetchCount(com.example.faktory.test.jooq.tables.Users.USERS)

            tx.rollbackToSavepoint("sp1")

            val countAfterRollback = txDsl.fetchCount(com.example.faktory.test.jooq.tables.Users.USERS)
